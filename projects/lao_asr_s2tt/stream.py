...
def callback(in_data, frame_count, time_info, status):
    ...
    )  # Append the chunk of audio data
...
while True:
    ...
    if len(audio_data_chunks) * CHUNK >= PROCESS_SAMPLES:
        ...
        if save_audio:
            wf, filename = open_new_file(
                0
            )  # Open a new file for the chunk if saving is enabled
        ...
        outputs = model.generate_and_decode(audio_chunks=[audio_data_tensor])
        print(outputs)
