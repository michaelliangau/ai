...
def create_hf_dataset(textgrid_folder, json_file):
    ...
    for additional_item in tqdm(additional_data):
        ...
        corresponding_dict = next((item for item in textgrid_data if os.path.splitext(item['path'])[0].split("/")[-1] == file_stem), None)
        ...
        additional_item["path"] = "/Users/michael/Desktop/librispeech_test_clean_other_raw/" + additional_item["path"]

    data_dict = {key: [dic[key] for dic in additional_data] for key in additional_data[0]}
...
# Define the paths
textgrid_folder = "/Users/michael/Desktop/wip/ai/projects/librispeech_data_processing/output_raw"
