...
policy_optimizer = torch.optim.Adam(
    actor_critic_agent.policy_network.parameters(), lr=learning_rate
)
value_optimizer = torch.optim.Adam(
    actor_critic_agent.value_network.parameters(), lr=learning_rate
)
num_training_steps = num_episodes
policy_scheduler = transformers.get_linear_schedule_with_warmup(
    policy_optimizer,
    num_warmup_steps=warmup_steps,
    num_training_steps=num_training_steps,
)
value_scheduler = transformers.get_linear_schedule_with_warmup(
    value_optimizer,
    num_warmup_steps=warmup_steps,
    num_training_steps=num_training_steps,
)
...
for episode in tqdm(range(num_episodes)):
    ...
    encoded_state = (
        actor_critic_agent.encode_sequence(sequence=state).unsqueeze(0).to(torch_device)
    )
    ...
    for i in tqdm(range(max_seq_length)):
        # Take action
        action, log_prob = actor_critic_agent.get_action_and_log_prob_rl(
            state=current_state, epsilon=exploration_epsilon
        )
    ...
    # Calculate losses
    policy_loss, value_loss = actor_critic_agent.compute_loss_ppo_rl(
        states=states,
        rewards=summed_rewards,
    ...
    rlhf_reward_perc = (
        abs(rlhf_rewards.item() * len(actions))
        / (abs(rlhf_rewards.item() * len(actions)) + abs(sum(rewards).item()))
    ) * 100
    print(f"Cumulative Reward: {cumulative_reward.item()}")
    print(f"RLHF Reward %: {rlhf_reward_perc}")
    print(
        f"Every 10th classifier reward value: {[rewards[i].item() for i in range(0, len(rewards), 10)]}"
    )
    ...
    common_utils.log_wandb(
        {
            ...
            "Learning Rate": policy_optimizer.param_groups[0]["lr"],
            "Cumulative Reward": cumulative_reward,
            "RLHF Reward %": rlhf_reward_perc,
        }
    )
