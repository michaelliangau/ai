...
def get_suburb_data(s, state):
    ...
    key_table_report_divs = parsed_html.body.find_all('div', attrs={'class':'key-table-report'})
    ...
    for idx,r in enumerate(tr):
        ...
        if idx == 1:
            data['house_median_price'] = str(td[1]).split("\">")[-1].split("</span>")[0].split("</label>")[-1].replace(",","")
            data['unit_median_price'] = str(td[2]).split("\">")[-1].split("</span>")[0].split("</label>")[-1].replace(",","")
        elif idx == 2:
            ...
            data['house_weekly_median_rent'] = str(td[1]).split("\">")[-1].split("</span>")[0].split("</label>")[-1].replace(",","")
            data['unit_weekly_median_rent'] = str(td[2]).split("\">")[-1].split("</span>")[0].split("</label>")[-1].replace(",","")   
        elif idx == 6:
            ...
            data['house_avg_days_on_market_12m'] = str(td[1]).split("\">")[-1].split("</")[0]
            data['unit_avg_days_on_market_12m'] = str(td[2]).split("\">")[-1].split("</")[0]  
    ...
    for idx,r in enumerate(tr):
        td = r.find_all("td")
        if idx == 1:
            data['2011_total_population'] = str(td[1]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0].replace(",","")
            data['2016_total_population'] = str(td[2]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0].replace(",","")
        elif idx == 2:
            data['2011_population_change_5y_pc'] = str(td[1]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0]
            data['2016_population_change_5y_pc'] = str(td[2]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0]
        elif idx == 3:
            data['2011_median_household_income_pw'] = str(td[1]).split("\">")[-1].split("</span>")[0].split("<span>")[-1].split("</label>")[-1].replace(",","").replace("$", "")
            data['2016_median_household_income_pw'] = str(td[2]).split("\">")[-1].split("</span>")[0].split("<span>")[-1].split("</label>")[-1].replace(",","").replace("$","")
        elif idx == 4:
            data['2011_household_income_change_5y_pc'] = str(td[1]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0]
            data['2016_household_income_change_5y_pc'] = str(td[2]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0]
        elif idx == 5:
            data['2011_median_age'] = str(td[1]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0]
            data['2016_median_age'] = str(td[2]).split("<span>")[-1].split("no-value\">")[-1].split("</span>")[0]
