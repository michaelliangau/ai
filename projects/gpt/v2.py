...
class Head(nn.Module):
    ...
    def __init__(self, head_size):
        ...
        )  # Buffer is used because tril is not a model parameter. It will not be stored in the state dict.
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, num_heads, head_size):
        ...
        )  # Regular python list cannot return parameters thus cannot be trained.
...
class BigramLanguageModel(nn.Module):
    ...
    def generate(self, idx, max_new_tokens):
        for _ in tqdm(range(max_new_tokens)):
            idx_cond = idx[
                :, -block_size:
            ]  # crop context so positioning embedding table doesn't error out
...
out = decode(
    m.generate(input, max_new_tokens=10000)[0].tolist()
