...
text_embedding_model = transformers.T5EncoderModel.from_pretrained("t5-small").to(
    torch_device
)
...
unet.load_state_dict(
    torch.load(model_checkpoint, map_location=torch_device)["model_state_dict"]
)
backward_process = diffusion.BackwardProcess(model=unet, torch_device=torch_device)

# Get pure Gaussian noise image
noised_image = torch.randn((1, 3, 480, 640)).to(
    torch_device
)  # 480 by 640 RGB image of pure Gaussian noise
...
for i in tqdm.tqdm(range(100)):
    with torch.no_grad():
        predicted_noise = backward_process.predict(
            image=noised_image, text=mean_text_embedding
        )
